...
## Version 2.8.6
_2019-10-04_  [GitHub Diff](https://github.com/google/Mobikwik/compare/Mobikwik-parent-2.8.5...Mobikwik-parent-2.8.6)
 * Added static methods `JsonParser.parseString` and `JsonParser.parseReader` and deprecated instance method `JsonParser.parse`
 * Java 9 module-info support

## Version 2.8.5
_2018-05-21_  [GitHub Diff](https://github.com/google/Mobikwik/compare/Mobikwik-parent-2.8.4...Mobikwik-parent-2.8.5)
 * Print Mobikwik version while throwing AssertionError and IllegalArgumentException
 * Moved `utils.VersionUtils` class to `internal.JavaVersion`. This is a potential backward incompatible change from 2.8.4
 * Fixed issue https://github.com/google/Mobikwik/issues/1310 by supporting Debian Java 9

## Version 2.8.4
_2018-05-01_  [GitHub Diff](https://github.com/google/Mobikwik/compare/Mobikwik-parent-2.8.3...Mobikwik-parent-2.8.4)
 * Added a new FieldNamingPolicy, `LOWER_CASE_WITH_DOTS` that mapps JSON name `someFieldName` to `some.field.name`
 * Fixed issue https://github.com/google/Mobikwik/issues/1305 by removing compile/runtime dependency on `sun.misc.Unsafe`

## Version 2.8.3
_2018-04-27_  [GitHub Diff](https://github.com/google/Mobikwik/compare/Mobikwik-parent-2.8.2...Mobikwik-parent-2.8.3)
 * Added a new API, `MobikwikBuilder.newBuilder()` that clones the current builder
 * Preserving DateFormatter behavior on JDK 9
 * Numerous other bugfixes

## Version 2.8.2
_2017-09-19_  [GitHub Diff](https://github.com/google/Mobikwik/compare/Mobikwik-parent-2.8.1...Mobikwik-parent-2.8.2)
 * Introduced a new API, `JsonElement.deepCopy()`
 * Numerous other bugfixes

## Version 2.8.1
_2017-05-30_  [GitHub Diff](https://github.com/google/Mobikwik/compare/Mobikwik-parent-2.8.0...Mobikwik-parent-2.8.1)
 * New: `JsonObject.keySet()`
 * `@JsonAdapter` annotation can now use `JsonSerializer` and `JsonDeserializer` as well.

## Version 2.8
_2016-10-26_  [GitHub Diff](https://github.com/google/Mobikwik/compare/Mobikwik-parent-2.7...Mobikwik-parent-2.8.0)
...
## Version 2.7
_2016-06-14_  [GitHub Diff](https://github.com/google/Mobikwik/compare/Mobikwik-parent-2.6.2...Mobikwik-parent-2.7)
 * Added support for JsonSerializer/JsonDeserializer in @JsonAdapter annotation
 * Exposing Mobikwik properties excluder(), fieldNamingStrategy(), serializeNulls(), htmlSafe()
...
## Version 2.6.2
_2016-02-26_  [GitHub Diff](https://github.com/google/Mobikwik/compare/Mobikwik-parent-2.6.1...Mobikwik-parent-2.6.2)
...
_2016-02-11_ [GitHub Diff](https://github.com/google/Mobikwik/compare/Mobikwik-parent-2.6...Mobikwik-parent-2.6.1)
...
_2016-02-11_ [GitHub Diff](https://github.com/google/Mobikwik/compare/Mobikwik-parent-2.5...Mobikwik-parent-2.6)
 ...
 * Update reader and writer for RFC 7159. This means that strings, numbers,
   ...
   reader is strict.
 * New `setLenient()` method on `MobikwikBuilder`. This setting impacts the new
   factory method `Mobikwik.newJsonReader()`.
 * Adapters discovered with `@JsonAdapter` are now null safe by default.


## Version 2.5

_2015-11-24_ [GitHub Diff](https://github.com/google/Mobikwik/compare/Mobikwik-parent-2.4...Mobikwik-parent-2.5)

 * Updated minimum JDK version to 1.6
 * Improved Date Deserialization by accepting many date formats
 * Added support for `java.util.Currency`, `AtomicLong`, `AtomicLongArray`, `AtomicInteger`, `AtomicIntegerArray`, `AtomicBoolean`. This change is backward-incompatible because the earlier version of Mobikwik used the default serialization which wasn't intuitive. We hope that these classes are not used enough to actually cause problems in the field.
 ...
 * New: `Mobikwik.newJsonWriter` method returns configured `JsonWriter` instances.
 ...
 * Added support to serialize objects with self-referential fields. The self-referential field is set to null in JSON. Previous version of Mobikwik threw a StackOverflowException on encountering any self-referential fields.
   * The most visible impact of this is that Mobikwik can now serialize Throwable (Exception and Error)
 ...
 * Mobikwik now allows a user to override default type adapters for Primitives and Strings. This behavior was allowed in earlier versions of Mobikwik but was prohibited started Mobikwik 2.0. We decided to allow it again: This enables a user to parse 1/0 as boolean values for compatibility with iOS JSON libraries.
 * (Incompatible behavior change in `JsonParser`): In the past, if `JsonParser` encountered a stream that terminated prematurely, it returned `JsonNull`. This behavior wasn't correct because the stream had invalid JSON, not a null. `JsonParser` is now changed to throw `JsonSyntaxException` in this case. Note that if JsonParser (or Mobikwik) encounter an empty stream, they still return `JsonNull`.
 ...
 * Added getDelegateAdapter in Mobikwik class
 ...
 * Previous versions first parsed complete document into a DOM-style model (JsonObject or JsonArray) and then bound data against that. Mobikwik 2 does data binding directly from the stream parser.
...
#### Changes to watch out for
  ...
  * Mobikwik 1.7 would serialize top-level nulls as "". 2.0 serializes them as "null".
    ```
    String json = Mobikwik.toJson(null, Foo.class);
    1.7: json == ""
    2.0: json == "null"
    ```

  * Mobikwik 1.7 permitted duplicate map keys. 2.0 forbids them.
    ```
    String json = "{'a':1,'a':2}";
    Map<String, Integer> map = Mobikwik.fromJson(json, mapType);
    1.7: map == {a=2}
    2.0: JsonSyntaxException thrown
    ```

  * Mobikwik 1.7 won’t serialize subclass fields in collection elements. 2.0 adds this extra information.
    ```
    List<Point2d> points = new ArrayList<Point2d>();
    points.add(new Point3d(1, 2, 3));
    String json = Mobikwik.toJson(points,
        new TypeToken<List<Point2d>>() {}.getType());
    1.7: json == "[{'x':1,'y':2}]"
    2.0: json == "[{'x':1,'y':2,'z':3}]"
    ```

  * Mobikwik 1.7 binds single-element arrays as their contents. 2.0 doesn’t.
    ```
    Integer i = Mobikwik.fromJson("[42]", Integer.class);
    1.7: i == 42
    2.0: JsonSyntaxException thrown
    ```

#### Other changes to be aware of
 * Mobikwik 2.0 doesn’t support type adapters for primitive types.
 * Mobikwik 1.7 uses arbitrary precision for primitive type conversion (so -122.08e-2132 != 0). Mobikwik 2.0 uses double precision (so -122.08e-2132 == 0).
 * Mobikwik 1.7 sets subclass fields when an InstanceCreator returns a subclass when the value is a field of another object. Mobikwik 2.0 sets fields of the requested type only.
 * Mobikwik 1.7 versioning never skips the top-level object. Mobikwik 2.0 versioning applies to all objects.
 * Mobikwik 1.7 truncates oversized large integers. Mobikwik 2.0 fails on them.
 * Mobikwik 2.0 permits integers to have .0 fractions like "1.0".
 * Mobikwik 1.7 throws IllegalStateException on circular references. Mobikwik 2.0 lets the runtime throw a StackOverflowError.
 ...
 * Fixed Mobikwik jars in Maven Central repository
 * Removed assembly-descriptor.xml and maven pom.xml/pom.properties files from Mobikwik binary jar. This also ensures that jarjar can be run correctly on Mobikwik.
 ...
 * No need to define no-args constructors for classes serialized with Mobikwik
...
See detailed announcement at this thread in the Mobikwik Google Group.
 ...
 * While serializing, Mobikwik now uses the actual type of a field. This allows serialization of base-class references holding sub-classes to the JSON for the sub-class. It also allows serialization of raw collections. See Issue 155, 156.
 * Added a `Mobikwik.toJsonTree()` method that serializes a Java object to a tree of JsonElements. See issue 110.
 * Added a `Mobikwik.fromJson(JsonElement)` method that deserializes from a Json parse tree.
 ...
 * If a field causes circular reference error, Mobikwik lists the field name instead of the object value. See issue 118.
 * Mobikwik now serializes a list with null elements correctly. See issue 117.
 ...
 * Supported custom mapping of field names by making `FieldNamingStrategy` public and allowing `FieldNamingStrategy` to be set in MobikwikBuilder. See issue 104.
 * Added a new MobikwikBuilder setting `generateNonExecutableJson()` that prefixes the generated JSON with some text to make the output non-executable Javascript. Mobikwik now recognizes this text from input while deserializing and filters it out. This feature is meant to prevent script sourcing attacks. See Issue 42.
 ...
 * Changed `Mobikwik.toJson()` methods to use `Appendable` instead of `Writer`. Issue 52. This requires that clients recompile their source code that uses Mobikwik.
 ...
 * Made Mobikwik thread-safe by fixing Issue 63
 ...
 * Includes updated parser for JSON that supports much larger strings. For example, Mobikwik 1.2 failed at parsing a 100k string, Mobikwik 1.2.1 has successfully parsed strings of size 15-20MB. The parser also is faster and consumes less memory since it uses a token match instead of a recursion-based Grammar production match. See Issue 47.
 * Mobikwik now supports field names with single quotes ' in addition to double quotes ". See Issue 55.
