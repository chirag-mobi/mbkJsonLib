...
import com.mobikwik.testcode.JsonElement;
import com.mobikwik.testcode.JsonPrimitive;
import com.mobikwik.testcode.JsonSyntaxException;
...
import com.mobikwik.testcode.internal.JsonReaderInternalAccess;
 ...
 *   }
 * }</pre>
 * But Mobikwik is unable to deserialize this value because the JSON string name is
 * just the {@link Object#toString() toString()} of the map key. Attempting to
 * convert the above JSON to an object fails with a parse exception:
 * <pre>JsonParseException: Expecting object found: "(5,6)"
 *   at com.google.Mobikwik.JsonObjectDeserializationVisitor.visitFieldUsingCustomHandler
 *   at com.google.Mobikwik.ObjectNavigator.navigateClassFields
 *   ...</pre>
 *
 ...
 *
 * <p>Register this adapter when you are creating your Mobikwik instance.
 * <pre>   {@code
...
public final class MapTypeAdapterFactory implements TypeAdapterFactory {
  ...
  @Override public <T> TypeAdapter<T> create(Mobikwik Mobikwik, TypeToken<T> typeToken) {
    ...
    Class<?> rawTypeOfSrc = $Mobikwik$Types.getRawType(type);
    Type[] keyAndValueTypes = $Mobikwik$Types.getMapKeyAndValueTypes(type, rawTypeOfSrc);
    TypeAdapter<?> keyAdapter = getKeyAdapter(Mobikwik, keyAndValueTypes[0]);
    TypeAdapter<?> valueAdapter = Mobikwik.getAdapter(TypeToken.get(keyAndValueTypes[1]));
    ObjectConstructor<T> constructor = constructorConstructor.get(typeToken);

    @SuppressWarnings({"unchecked", "rawtypes"})
    // we don't define a type parameter for the key or value types
    TypeAdapter<T> result = new Adapter(Mobikwik, keyAndValueTypes[0], keyAdapter,
   ...
   */
  private TypeAdapter<?> getKeyAdapter(Mobikwik context, Type keyType) {
  ...
  private final class Adapter<K, V> extends TypeAdapter<Map<K, V>> {
    ...
    public Adapter(Mobikwik context, Type keyType, TypeAdapter<K> keyTypeAdapter,
