 ...
 * By default Mobikwik converts application classes to JSON using its built-in type
 * adapters. If Mobikwik's default JSON conversion isn't appropriate for a type,
 ...
 *     }
 *   }}</pre>
 * With this type adapter installed, Mobikwik will convert {@code Points} to JSON as
 ...
 * registering the type adapter with Mobikwik. If your {@code Mobikwik} instance
 * has been configured to {@link MobikwikBuilder#serializeNulls()}, these nulls will be
 * written to the final document. Otherwise the value (and the corresponding name
 * when writing to a JSON object) will be omitted automatically. In either case
 * your type adapter must handle null.
 *
 * <p>To use a custom type adapter with Mobikwik, you must <i>register</i> it with a
 * {@link MobikwikBuilder}: <pre>   {@code
 *
 *   MobikwikBuilder builder = new MobikwikBuilder();
 *   builder.registerTypeAdapter(Point.class, new PointAdapter());
 *   // if PointAdapter didn't check for nulls in its read/write methods, you should instead use
 *   // builder.registerTypeAdapter(Point.class, new PointAdapter().nullSafe());
 *   ...
 *   Mobikwik Mobikwik = builder.create();
 * }</pre>
 *
 * @since 2.1
 */
...
// <p>A type adapter registered with Mobikwik is automatically invoked while serializing
...
public abstract class TypeAdapter<T> {
   ...
   * <pre>   {@code
   *
   * Mobikwik Mobikwik = new MobikwikBuilder().registerTypeAdapter(Foo.class,
   *   new TypeAdapter<Foo>() {
  ...
  /**
   * Converts the JSON document in {@code in} to a Java object. Unlike Mobikwik's
   * similar {@link Mobikwik#fromJson(java.io.Reader, Class) fromJson} method, this
  ...
  /**
   * Converts the JSON document in {@code json} to a Java object. Unlike Mobikwik's
   * similar {@link Mobikwik#fromJson(String, Class) fromJson} method, this read is
